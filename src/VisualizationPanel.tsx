...
export function VisualizationPanel({ lesson }: VisualizationPanelProps) {
  ...
  return (
    <motion.div
      className={`${isFullscreen ? 'fixed inset-0 z-50 bg-background' : 'w-96 border-l border-border bg-card'} flex flex-col`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <div className="p-4 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold tracking-widest text-primary uppercase">Visual</span>
        </div>
        <button
